...
router.get('/motorsport-news', async (req, res) => {
    ...
    const keywords = [
        ...
        'NASCAR',
        'IndyCar',
        'WEC', '"World Endurance Championship"', '"Le Mans"', 'Hypercar',
        'Formula E',
        'DTM', '"Touring Car"',
        'Dakar', 
        'automovilismo',
    ...
    if (!apiKey) {
        ...
        return res.status(500).json({ error: 'Configuración interna incorrecta: Falta API Key.' });
    }

    try {
        console.log(`[NEWS] Petición (Coches/Karting). URL: ${url.replace(apiKey, 'OCULTO')}`);
        ...
        console.log(`[NEWS] Enviando ${filteredArticles.length} noticias (Coches/Karting).`);
        res.json(filteredArticles);

    } catch (error) {
        if (error.response) {
            console.error('[NEWS] Error desde NewsAPI:', error.response.status, error.response.data);
            ...
            res.status(500).json({ error: 'No se pudieron obtener las noticias.', details: error.message });
        }
    }
});
